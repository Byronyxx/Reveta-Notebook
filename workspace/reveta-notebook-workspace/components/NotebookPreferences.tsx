...
import {
  CHAT_LANGUAGES,
  ResponseLength, ResponseFormality, ResponseFormat, StylePreference,
  DEFAULT_STYLE,
} from '@/lib/ai/prompts'

// ── TYPES ─────────────────────────────────────────────────────────────────────

interface NotebookPrefs {
      ...
      <div className="flex flex-wrap gap-reveta-2">
        {options.map(opt => (
          <button
            ...
            className={[
              'group flex flex-col items-start px-reveta-3 py-reveta-2 rounded-component border',
              'transition-all duration-quick text-left min-w-[120px]',
              value === opt.value
                ? 'surface-brand-border bg-primary-800 border-primary-600 text-primary-100'
                : 'surface-ground border-border-default text-text-secondary hover:border-neutral-600 hover:text-text-primary',
            ].join(' ')}
          ...
          >
            ...
            <span className={[
              'reveta-label mt-0.5',
              value === opt.value ? 'text-primary-300' : 'text-text-secondary',
            ].join(' ')}>{opt.hint}</span>
          </button>
        ))}
      </div>
...
export default function NotebookPreferences({
  notebookId,
  initialPrefs,
  onClose,
  onSave,
  ...
  notebookId: string
  initialPrefs: NotebookPrefs
  onClose: () => void
  onSave: (prefs: NotebookPrefs) => void
}) {
  const [style, setStyle] = useState<StylePreference>({
    ...DEFAULT_STYLE,
    ...initialPrefs.stylePreference,
  })
  const [language, setLanguage] = useState(initialPrefs.responseLanguage || 'en')
  const [saving, setSaving] = useState(false)
  const [saved, setSaved] = useState(false)

  const updateStyle = useCallback(<K extends keyof StylePreference>(
    key: K, value: StylePreference[K]
  ) => {
  ...
  const handleSave = async () => {
    ...
    if (res.ok) {
      setSaved(true)
      onSave({ stylePreference: { ...style, language }, responseLanguage: language })
      setTimeout(() => setSaved(false), 2500)
    }
  }

  const isDirty =
    style.length !== (initialPrefs.stylePreference?.length || DEFAULT_STYLE.length) ||
    style.formality !== (initialPrefs.stylePreference?.formality || DEFAULT_STYLE.formality) ||
    style.format !== (initialPrefs.stylePreference?.format || DEFAULT_STYLE.format) ||
    language !== (initialPrefs.responseLanguage || 'en')
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-reveta-4">
      ...
      <div
        className="relative surface-float rounded-large w-full max-w-2xl shadow-2xl border border-border-default flex flex-col max-h-[90vh]"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="flex items-center justify-between px-reveta-5 py-reveta-4 border-b border-border-default shrink-0">
          <div>
            ...
            <p className="reveta-label text-text-secondary mt-0.5">Chat response style and language — FR-11 · FR-18</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-reveta-5 py-reveta-5 flex flex-col gap-reveta-6">
          ...
          <section>
            ...
            <div className="flex flex-col gap-reveta-5">
              <ChipSelector
                ...
                value={style.length}
                onChange={v => updateStyle('length', v)}
              />
              <ChipSelector
                label="Tone"
              ...
              <ChipSelector
                ...
                value={style.format}
                onChange={v => updateStyle('format', v)}
              />
            </div>
          </section>

          {/* FR-18 — Response Language */}
          <section>
            <h3 className="reveta-h4 text-text-primary mb-reveta-2">Response Language</h3>
            <p className="reveta-caption text-text-secondary mb-reveta-3">
              All AI chat responses will be delivered in the selected language, regardless of the query language.
            </p>
            <select
              value={language}
              onChange={e => { setLanguage(e.target.value); setSaved(false) }}
              className={[
                'w-full h-10 px-reveta-3 surface-ground border border-border-default rounded-component',
                'reveta-caption text-text-primary',
          ...
          {/* Live preview */}
          <section>
            <h3 className="reveta-h4 text-text-primary mb-reveta-2">Active directive preview</h3>
            <div className="surface-ground rounded-component p-reveta-3 border border-border-default">
              <p className="reveta-label font-mono text-signal-300 whitespace-pre-wrap text-xs leading-relaxed">
                {`LENGTH: ${LENGTH_OPTIONS.find(o => o.value === style.length)?.hint}
TONE: ${FORMALITY_OPTIONS.find(o => o.value === style.formality)?.hint}
FORMAT: ${FORMAT_OPTIONS.find(o => o.value === style.format)?.hint}${language !== 'en' ? `\nLANGUAGE: ${CHAT_LANGUAGES.find(l => l.code === language)?.label}` : ''}`}
              </p>
            </div>
          </section>
        </div>
        ...
        <div className="flex items-center justify-between px-reveta-5 py-reveta-4 border-t border-border-default shrink-0">
          <p className="reveta-label text-text-secondary">
            Settings apply to all subsequent chats in this notebook.
          </p>
